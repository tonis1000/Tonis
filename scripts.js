...
// Event-Listener für Klicks auf Vorheriges Datum
document.querySelector('.datePrev').addEventListener('click', function() {
    // Heute abrufen
    const currentDate = new Date();
    // Einen Tag von heute subtrahieren
    const previousDate = new Date(currentDate);
    previousDate.setDate(currentDate.getDate() - 1);
    // Neues Datum im gewünschten Format anzeigen
    updateDate(formatDate(previousDate));
});

// Event-Listener für Klicks auf Nächstes Datum
document.querySelector('.dateNext').addEventListener('click', function() {
    // Heute abrufen
    const currentDate = new Date();
    // Einen Tag zu heute addieren
    const nextDate = new Date(currentDate);
    nextDate.setDate(currentDate.getDate() + 1);
    // Neues Datum im gewünschten Format anzeigen
    updateDate(formatDate(nextDate));
});
...
function showPopup(title, text) {
    const popup = document.querySelector('.programPopup');
    const titleElement = popup.querySelector('.title');
    const textElement = popup.querySelector('.text');
...
// Beispielaufruf: showPopup('Titel des Popups', 'Inhalt des Popups');
